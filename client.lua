...
AddEventHandler('ws_sellshop:sellItem', function(data)
    local data = data
    local input = lib.inputDialog('Koliko jih želite prodati?', {'Količina'})
    if input then
        data.quantity = math.floor(tonumber(input[1]))
        if data.quantity < 1 then
            lib.notify({
                title = 'Napaka',
                description = 'Vnesite veljaven znesek!',
                type = 'error'
            })
        else
            ...
            if not done then
                lib.notify({
                    title = 'Napaka',
                    description = 'Nimate zahtevanih artiklov za prodajo!',
                    type = 'error'
                })
            else
                lib.notify({
                    title = 'Uspeh',
                    description = 'Prodali ste vaše artikle in zaslužili €'..addCommas(done),
                    type = 'success'
                })
            end
        end
...
AddEventHandler('ws_sellshop:interact', function(data)
    ...
    for i=1, #items do
        table.insert(Options, {
            title = items[i].label,
            description = 'Prodajna cena: €'..items[i].price,
